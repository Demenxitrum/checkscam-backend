...
public class LookupServiceImpl implements LookupService {
    ...
    // ===================================
    // ✅ UNIFIED LOOKUP (GET /api/lookup?type=&value=)
    // ===================================
    @Override
    public LookupResponse lookupByType(String type, String value) {
        if (type == null || value == null) {
            throw new RuntimeException("Type hoặc value không được null");
        }

        return switch (type.toUpperCase()) {
            case "PHONE" -> lookupPhone(value);
            case "BANK" -> lookupBank(value);
            case "URL" -> lookupURL(value);
            default -> throw new RuntimeException("Loại lookup không hợp lệ: " + type);
        };
    }
    ...
    // ===================================
    // LOOKUP GENERIC (POST /api/lookup)
    // ===================================
    @Override
    public LookupResponse lookupGeneric(LookupRequest req) {
        if (req == null || req.getType() == null || req.getValue() == null) {
            throw new RuntimeException("LookupRequest không hợp lệ");
        }

        return lookupByType(req.getType(), req.getValue());
    }
}
